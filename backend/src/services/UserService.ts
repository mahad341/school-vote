...
export class UserService {
  ...
  async getUsers(options: {
    ...
    if (search) {
      queryBuilder.andWhere(
        '(user.firstName ILIKE :search OR user.lastName ILIKE :search OR user.studentId ILIKE :search OR user.email ILIKE :search)',
        { search: `%${search}%` }
      );
    }
   ...
   */
  async updateUser(id: string, updateData: UpdateUserData, updatedBy: string): Promise<User> {
    ...
    if (updateData.house !== undefined) user.house = updateData.house;
    if (updateData.class !== undefined) user.class = updateData.class;
   ...
   */
  async importUsers(csvData: CreateUserData[], importedBy: string): Promise<{
    success: number;
    failed: number;
    errors: string[];
  }> {
   ...
   */
  async resetVotingStatus(resetBy: string): Promise<number> {
   ...
   * Update user role
   */
  async updateUserRole(id: string, role: UserRole, updatedBy: string): Promise<User> {
   ...
   * Update user status
   */
  async updateUserStatus(id: string, status: UserStatus, updatedBy: string, reason?: string): Promise<User> {
