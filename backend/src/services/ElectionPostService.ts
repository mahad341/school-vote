...
export class ElectionPostService {
   ...
   */
  async getPostById(id: string, options: { includeCandidates?: boolean; includeResults?: boolean } = {}): Promise<ElectionPost | null> {
    const queryBuilder = this.postRepository.createQueryBuilder('post');
   ...
   */
  async createPost(postData: CreatePostData, createdBy: string): Promise<ElectionPost> {
   ...
   */
  async updatePost(id: string, updateData: UpdatePostData, updatedBy: string): Promise<ElectionPost> {
    ...
    if (updateData.description !== undefined) post.description = updateData.description;
    if (updateData.type !== undefined) post.type = updateData.type;
    if (updateData.eligibleHouses !== undefined) post.eligibleHousesArray = updateData.eligibleHouses;
    if (updateData.maxVotes !== undefined) post.maxVotes = updateData.maxVotes;
    if (updateData.votingStartsAt !== undefined) post.votingStartsAt = updateData.votingStartsAt;
    if (updateData.votingEndsAt !== undefined) post.votingEndsAt = updateData.votingEndsAt;
    if (updateData.status !== undefined) post.status = updateData.status;
    if (updateData.displayOrder !== undefined) post.displayOrder = updateData.displayOrder;
   ...
   */
  async setPostStatus(id: string, status: PostStatus, updatedBy: string): Promise<ElectionPost> {
    const post = await this.postRepository.findOne({ where: { id } });
   ...
   */
  async getPostResults(id: string): Promise<{
    post: ElectionPost;
    candidates: Array<{
      id: string;
      firstName: string;
   ...
   */
  async getActivePosts(userHouse?: string): Promise<ElectionPost[]> {
