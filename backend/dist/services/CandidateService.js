...
export class CandidateService {
    ...
    async getCandidates(options = {}) {
        ...
        if (search) {
            queryBuilder.andWhere('(candidate.firstName ILIKE :search OR candidate.lastName ILIKE :search OR candidate.studentId ILIKE :search)', { search: `%${search}%` });
        }
     ...
     * Get candidate by ID
     */
    async getCandidateById(id, includePost = false) {
        const queryBuilder = this.candidateRepository.createQueryBuilder('candidate');
        if (includePost) {
            queryBuilder.leftJoinAndSelect('candidate.post', 'post');
        }
     ...
     */
    async updateCandidate(id, updateData, updatedBy) {
        ...
        if (updateData.firstName !== undefined)
            candidate.firstName = updateData.firstName;
        if (updateData.lastName !== undefined)
            candidate.lastName = updateData.lastName;
        if (updateData.studentId !== undefined)
            candidate.studentId = updateData.studentId;
        if (updateData.email !== undefined)
            candidate.email = updateData.email;
        if (updateData.bio !== undefined)
            candidate.bio = updateData.bio;
        if (updateData.manifesto !== undefined)
            candidate.manifesto = updateData.manifesto;
        if (updateData.house !== undefined)
            candidate.house = updateData.house;
        if (updateData.class !== undefined)
            candidate.class = updateData.class;
        if (updateData.status !== undefined)
            candidate.status = updateData.status;
     ...
     * Delete candidate
     */
    async deleteCandidate(id, deletedBy) {
        const candidate = await this.candidateRepository.findOne({
            where: { id },
     ...
     */
    async uploadCandidatePhoto(id, photoFile, uploadedBy) {
     ...
     */
    async getCandidatesForPost(postId, userHouse) {
