...
export class RealtimeController {
     ...
     */
    static async getLiveResults(req, res) {
        try {
     ...
     */
    static async getAllLiveResults(req, res) {
        try {
            const postService = new ElectionPostService();
            const voteService = new VoteService();
            // Get all active posts
            const posts = await postService.getPosts({ status: 'active', includeCandidates: true });
     ...
     */
    static async broadcastResults(req, res) {
        try {
            ...
            if (postId) {
                ...
                const service = new ElectionPostService();
                const posts = await service.getPosts({ status: 'active' });
                await emitElectionResults({
